...
export async function snacksRoutes(app: FastifyInstance) {
  app.post(
    ...
    async (request, response) => {
      ...
      // A partir deste sessionID, buscar os dados na tabela users para adicionar durante a criação de uma nova refeição na tabela snacks
      ...
      // Após a identificação do usuário, armazendo o dado de seu id para posteriormente adicionar na tabela de snacks junto ao prato
  ...
  app.get("/", { preHandler: [checkSessionIdExists] }, async (request) => {
    ...
    const snacks = await knex("snacks").where("user_id", userId).select();
  ...
  app.get(
    ...
    async (request, response) => {
      ...
      // Buscando na tabela snacks, na coluna ID, o params.id (que é o que vem da rota)
      // .first() é para não retornar como array e sim como (existendo ou undefined)
      const meal = await knex("snacks")
        .where("id", params.id)
  ...
  app.get(
    ...
    async (request) => {
      ...
      const refDieta = await knex("snacks")
        .count("id", { as: "Total de refeições dentro da dieta" })
        .where("is_diet", true)
        .andWhere("user_id", userId);

      const refForaDieta = await knex("snacks")
